...
def add(acc, kcal, protein, carb):
    acc["kcal"] += kcal; acc["protein"] += protein; acc["carb"] += carb

def from100(key, grams):
    v = P100[key]; f = grams / 100.0
    return v["kcal"]*f, v["protein"]*f, v["carb"]*f

def compute(name):
    """returns (category, nutrition|None, basis_list)"""
    acc = {"kcal": 0.0, "protein": 0.0, "carb": 0.0}
    basis = []
    mk = meat_key(name)
    ...
    if "카레라이스" in name:
        ...
        cs = PS["curry_sauce"]; add(acc, cs["kcal"], cs["protein"], cs["carb"]); basis.append("일본식 카레소스 1인분")
    ...
    # 음료
    if "아메리카노" in name:
        v = PS["americano"]; return "음료", {"kcal": v["kcal"], "protein": v["protein"], "carb": v["carb"]}, ["아메리카노 벤티"]
    if "제로" in name or "콜라" in name or "스프라이트" in name or "에너지드링크" in name:
        v = PS["zero_drink"]; return "음료", {"kcal": v["kcal"], "protein": v["protein"], "carb": v["carb"]}, ["제로 음료(제로칼로리 표기)"]
...
for e in out:
    n = e["nutrition"]
    s = f'{n["kcal"]}kcal P{n["protein"]} C{n["carb"]}' if n else "정보 확인 중"
